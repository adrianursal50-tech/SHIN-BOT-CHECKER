#!/usr/bin/env python3
"""
MLBB CN31 Telegram Bot - Production Ready
- 3 separate buttons: Lookup, Ban Check, Creation Date
- 5 free checks per day per user (admin unlimited)
- Hardcoded token & admin ID
- Guards against NoneType errors and registers error handler
"""
...
# ============================================================
# DAILY LIMIT TRACKING (in-memory – resets on bot restart)
# ============================================================
user_daily_usage = {}
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                if data.get("ban_status", 0) > 0:
                    return True, data.get("ban_reason", "Permanent Ban")
...
# ============================================================
# ACTION FUNCTIONS (executed in threads)
# ============================================================
...
# ============================================================
# BOT HANDLERS (with guards against NoneType errors)
# ============================================================
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await query.answer()
    if not update.effective_user:
        await query.edit_message_text("❌ User not identified.")
        return
    action = query.data
...
async def handle_device_id(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    # Guard: ignore updates without a user or without a message
    if not update.effective_user:
        return
...
# ============================================================
# ERROR HANDLER (catches any unexpected exceptions)
# ============================================================
...
def main() -> None:
    ...
    app.add_error_handler(error_handler)   # <-- CRITICAL ADD
